...
$usage = LicenceKitFacade::register(
    $license,
...
```bash
php artisan laranail::license-kit.keys.issue-signing --scope erp-system --kid erp-key-2024
```
...
'services' => [
    'key_generator' => \Simtabi\Laranail\Licence\Kit\Services\EncryptedLicenseKeyGenerator::class,
    'key_retriever' => \Simtabi\Laranail\Licence\Kit\Services\EncryptedLicenseKeyRetriever::class,
    'key_regenerator' => \Simtabi\Laranail\Licence\Kit\Services\EncryptedLicenseKeyRegenerator::class,
],
```
...
| Package | Description |
|---------|-------------|
| [laranail/license-verifier](https://github.com/laranail/license-verifier) | Headless client for validating licenses against a server — offline verification, device fingerprinting, usage/seats, and route middleware |
| [laranail/license-verifier-ui](https://github.com/laranail/license-verifier-ui) | Blade, Filament, Livewire and Vue UI presets for the verifier, in one package |
...
> **Heads up:** `boost:update --discover` uses an interactive multi-select. On a TTY, select `laranail/license-kit` when prompted. In non-interactive environments (CI, automation), the prompt is silently skipped and the package is **not** added — append it manually to `boost.json`:
>
> ```json
> {
>     "packages": ["laranail/license-kit"]
> }
> ```
>
...
If you discover a security vulnerability, please email opensource@simtabi.com instead of using the issue tracker. See [SECURITY.md](SECURITY.md).
