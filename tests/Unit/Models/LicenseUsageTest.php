...
test('can create usage with auto timestamps', function (): void {
...
test('can revoke usage', function (): void {
    ...
    Event::assertDispatched(UsageRevoked::class, fn ($event): bool => $event->usage->id === $usage->id && $event->reason === 'test reason');
});

test('cannot revoke already revoked usage', function (): void {
    $usage = $this->createUsage($this->license);
    $usage->revoke();
...
test('can check if usage is active', function (): void {
...
test('can check if usage is stale', function (): void {
...
test('stale check returns false when policy disabled', function (): void {
    $license = $this->createLicense([
...
test('can calculate days since last seen', function (): void {
    $usage = $this->createUsage($this->license, [
...
test('scope revoked filters correctly', function (): void {
...
test('scope stale filters by days', function (): void {
...
test('scope for fingerprint finds correct usage', function (): void {
