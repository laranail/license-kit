...
// Regression test for: https://github.com/laranail/license-kit/issues/3
//
...
test('resolvePassphrase works when passphrase is in config and env superglobals are unavailable', function (): void {
...
test('resolvePassphrase throws when passphrase is not configured', function (): void {
    config()->set('licensing.crypto.keystore.passphrase');
    LicensingKey::forgetCachedPassphrase();

    expect(fn (): LicensingKey => (new LicensingKey)->generate(['type' => KeyType::Root]))
        ->toThrow(RuntimeException::class, 'Key passphrase not configured');
});

test('encrypt v2 and decrypt v2 round-trip', function (): void {
    $key = (new LicensingKey)->generate(['type' => KeyType::Root]);
...
test('decrypt v1 legacy format for backward compatibility', function (): void {
...
test('decrypt v1 legacy format whose nonce starts with the v2 version byte', function (): void {
...
test('decrypt fails with wrong passphrase', function (): void {
    $key = (new LicensingKey)->generate(['type' => KeyType::Root]);
    ...
    expect(fn (): ?string => $key->getPrivateKey())
        ->toThrow(RuntimeException::class, 'Failed to decrypt private key');
});

test('sodium_memzero cleanup does not cause errors on repeated operations', function (): void {
    $key = (new LicensingKey)->generate(['type' => KeyType::Root]);
...
test('forgetCachedPassphrase clears the static cache', function (): void {
