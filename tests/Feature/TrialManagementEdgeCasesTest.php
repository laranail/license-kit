...
it('handles trial with zero duration days gracefully', function (): void {
...
it('prevents conversion of expired trial', function (): void {
...
it('prevents conversion of cancelled trial', function (): void {
    $trial = $this->trialService->startTrial($this->license, 'device-1', 7);
    $trial->cancel();
...
it('handles concurrent trial registrations safely', function (): void {
...
it('correctly calculates days remaining for trial', function (): void {
...
it('handles empty limitations and restrictions', function (): void {
...
it('handles null values in trial metadata', function (): void {
    $trial = $this->trialService->startTrial($this->license, 'device-1', 7);
...
it('verifies fingerprint hashing is consistent', function (): void {
...
it('can verify legacy SHA256 fingerprints from older versions', function (): void {
...
it('handles trial expiration at exact boundary', function (): void {
    $trial = $this->trialService->startTrial($this->license, 'device-1', 7);
...
it('prevents starting trial on suspended license', function (): void {
...
it('handles very long extension reasons gracefully', function (): void {
    $trial = $this->trialService->startTrial($this->license, 'device-1', 7);
    ...
    expect($extendedTrial->extension_reason)->toBe($longReason)
        ->and(strlen((string) $extendedTrial->extension_reason))->toBeGreaterThan(1000);
});

it('correctly handles trial with future start date', function (): void {
    $trial = $this->trialService->startTrial($this->license, 'device-1', 7);
...
it('handles conversion with zero value', function (): void {
    $trial = $this->trialService->startTrial($this->license, 'device-1', 7);
