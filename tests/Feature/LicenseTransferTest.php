...
it('can initiate a license transfer', function (): void {
...
it('validates transfer eligibility', function (): void {
...
it('creates required approvals based on transfer type', function (): void {
    $transfer = $this->transferService->initiateTransfer(
        $this->license,
        $this->targetUser,
        TransferType::UserToUser,
...
it('executes transfer when all approvals are received', function (): void {
    Event::fake([
...
it('rejects transfer when approval is denied', function (): void {
    Event::fake([
...
it('preserves usages when configured', function (): void {
    // Create some usages
...
it('revokes usages when not preserved', function (): void {
    // Create some usages
    $usage1 = $this->license->usages()->create([
        'usage_fingerprint' => 'device-1',
        'status' => 'active',
...
it('creates immutable transfer history', function (): void {
    $transfer = $this->transferService->initiateTransfer(
        $this->license,
        $this->targetUser,
        TransferType::UserToUser,
...
it('enforces cooling period between transfers', function (): void {
...
it('detects ping-pong transfer pattern', function (): void {
...
it('detects frequent transfer pattern', function (): void {
    config(['licensing.transfer.suspicious_pattern_requires_review' => true]);
...
it('allows admin to override transfer approval', function (): void {
    ...
    $adminUser->hasPermission = fn ($permission): bool => $permission === 'approve-license-transfers';
...
it('can cancel pending transfer', function (): void {
    $transfer = $this->transferService->initiateTransfer(
        $this->license,
        $this->targetUser,
        TransferType::UserToUser,
...
it('expires stale transfer requests', function (): void {
    $transfer = $this->transferService->initiateTransfer(
        $this->license,
        $this->targetUser,
        TransferType::UserToUser,
...
it('validates approval token', function (): void {
    $transfer = $this->transferService->initiateTransfer(
        $this->license,
        $this->targetUser,
        TransferType::UserToUser,
...
it('resets activation when configured', function (): void {
...
it('preserves expiration when configured', function (): void {
...
it('updates expiration when not preserved', function (): void {
    $originalExpiration = now()->addYear();
...
it('generates unique transfer codes', function (): void {
    $transfer1 = $this->transferService->initiateTransfer(
        $this->license,
        $this->targetUser,
        TransferType::UserToUser,
