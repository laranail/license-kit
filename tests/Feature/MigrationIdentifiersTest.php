 ...
 * regressions like https://github.com/laranail/license-kit/issues/9
...
it('keeps every table name within the MySQL identifier limit', function (): void {
    ...
    foreach ($tables as $table) {
        expect(strlen((string) $table))
            ->toBeLessThanOrEqual(MYSQL_IDENTIFIER_MAX, "Table name '{$table}' exceeds MySQL's 64-char limit");
    }
});

it('keeps every index name within the MySQL identifier limit', function (): void {
    if (DB::connection()->getDriverName() !== 'sqlite') {
        $this->markTestSkipped('Identifier length is enforced by the DB engine on non-sqlite drivers.');
    }
...
it('can run the real package migrations in the order declared by the service provider', function (): void {
