...
it('can create a license with key', function (): void {
...
it('can retrieve a license key when enabled', function (): void {
...
it('cannot retrieve a license key when disabled', function (): void {
...
it('can regenerate a license key', function (): void {
...
it('stores previous key hashes when regenerating', function (): void {
    $license = License::createWithKey();
    $originalHash = $license->key_hash;
...
it('cannot regenerate key when disabled', function (): void {
    ...
    expect(fn (): string => $license->regenerateKey())
        ->toThrow(RuntimeException::class, 'License key regeneration is not available');
});

it('can verify a license key', function (): void {
    $license = License::createWithKey();
...
it('can find a license by key', function (): void {
    $license = License::createWithKey();
    $key = $license->license_key;
...
it('returns null when finding non-existent key', function (): void {
    $found = License::findByKey('non-existent-key');

    expect($found)->toBeNull();
});

it('encrypts stored keys properly', function (): void {
    $license = License::createWithKey();
    $key = $license->license_key;
...
it('can create license with provided key', function (): void {
...
it('can create license with provided key and store it encrypted', function (): void {
    $providedKey = 'CUSTOM-KEY-1234-5678';
...
it('auto generates key when not provided', function (): void {
    $license = License::createWithKey([
        'max_usages' => 2,
    ]);
...
it('retrieves the exact same key that was stored', function (): void {
    config(['licensing.key_management.retrieval_enabled' => true]);
...
it('encrypt and decrypt preserve exact key format', function (): void {
...
it('handles special characters in license key', function (): void {
    $specialKey = 'KEY!@#$%^&*()_+-=[]{}|;:,.<>?';
...
it('persists encrypted key across database operations', function (): void {
    $license = License::createWithKey();
    $originalKey = $license->license_key;
...
it('maintains key integrity after regeneration', function (): void {
    $license = License::createWithKey();
...
it('createWithKey accepts provided key', function (): void {
...
it('createWithKey generates key when not provided', function (): void {
    $license = License::createWithKey([
        'max_usages' => 5,
    ]); // No key provided
