...
test('can register new usage', function (): void {
...
test('returns existing usage for same fingerprint', function (): void {
    $fingerprint = $this->generateFingerprint();
...
test('re-registers a previously revoked fingerprint by reusing the same row', function (): void {
    $fingerprint = $this->generateFingerprint();
...
test('updates heartbeat for existing usage', function (): void {
    testTime()->freeze();
    $fingerprint = $this->generateFingerprint();
    $usage = $this->registrar->register($this->license, $fingerprint);
...
test('auto replaces oldest usage when limit reached', function (): void {
    testTime()->freeze();
    $this->license->update([
        'max_usages' => 2,
...
test('emits usage limit reached event', function (): void {
    ...
    Event::assertDispatched(UsageLimitReached::class, fn ($event): bool => $event->license->id === $this->license->id
        && $event->fingerprint === 'fingerprint2');
});

test('can check if registration is allowed', function (): void {
...
test('prevents double registration once seat limit is hit', function (): void {
    $this->license->update(['max_usages' => 1]);
...
test('respects global fingerprint uniqueness', function (): void {
...
test('respects per-license fingerprint scope', function (): void {
...
test('can revoke usage', function (): void {
    $usage = $this->registrar->register($this->license, 'fingerprint');
...
test('can update heartbeat', function (): void {
    testTime()->freeze();
    $usage = $this->registrar->register($this->license, 'fingerprint');
...
test('cannot heartbeat revoked usage', function (): void {
    $usage = $this->registrar->register($this->license, 'fingerprint');
...
test('registers usage without request binding when metadata provided', function (): void {
