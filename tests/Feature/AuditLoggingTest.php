...
test('logs license creation', function (): void {
    $license = $this->createLicense();

    $log = LicensingAuditLog::where('auditable_type', $license::class)
...
test('logs license activation', function (): void {
...
test('logs usage registration', function (): void {
    $usage = $this->createUsage($this->license);

    $log = LicensingAuditLog::where('auditable_type', $usage::class)
...
test('logs usage revocation with reason', function (): void {
    $usage = $this->createUsage($this->license);
...
test('logs key generation', function (): void {
    $key = $this->createRootKey();

    $log = LicensingAuditLog::where('auditable_type', $key::class)
...
test('logs key rotation', function (): void {
...
test('logs license renewal', function (): void {
...
test('logs usage limit reached', function (): void {
...
test('can query audit logs by time range', function (): void {
...
test('can disable audit logging', function (): void {
...
test('stores actor information when available', function (): void {
    ...
    expect($log)->not->toBeNull()
        ...
        ->and($log->meta['actor']['actor_type'])->toBe($user::class)
    ...
    $log2 = LicensingAuditLog::where('auditable_type', $license2::class)
