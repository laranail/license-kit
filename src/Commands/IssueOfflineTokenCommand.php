...
class IssueOfflineTokenCommand extends Command
{
    ...
    public function handle(PasetoTokenService $tokenService): int
    {
        ...
        if (! $licenseRef || ! $fingerprint) {
            $this->line(__('laranail-license-kit::license-kit.offline_token.args_required'));
        ...
        if (! $license) {
            $this->line(__('laranail-license-kit::license-kit.offline_token.license_not_found', ['license' => $licenseRef]));
        ...
        if (! $usage) {
            $this->line(__('laranail-license-kit::license-kit.offline_token.no_usage', ['fingerprint' => $fingerprint]));
        ...
        if (! $signingKey instanceof LicensingKey) {
            $this->line(__('laranail-license-kit::license-kit.offline_token.no_signing'));

            return 3;
        }

        if ($signingKey->isRevoked()) {
            $this->line(__('laranail-license-kit::license-kit.offline_token.signing_revoked'));
        ...
        try {
            ...
            $this->line(__('laranail-license-kit::license-kit.offline_token.issued'));
            $this->line(__('laranail-license-kit::license-kit.offline_token.token_label'));
            $this->line($token);

            return 0;
        } catch (RuntimeException $e) {
            // Crypto/key errors return code 3
            $this->line(__('laranail-license-kit::license-kit.offline_token.failed', ['error' => $e->getMessage()]));
