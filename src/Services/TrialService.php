...
class TrialService
{
    ...
    {
        return DB::transaction(fn (): License => $trial->convert($trigger, $value));
    }

    public function extendTrial(LicenseTrial $trial, int $days, ?string $reason = null): LicenseTrial
    {
        return DB::transaction(fn (): LicenseTrial => $trial->extend($days, $reason));
    }
    ...
    public function checkExpiredTrials(): int
    {
        ...
        LicenseTrial::where('status', TrialStatus::Active)
            ...
            ->chunk(100, function ($trials) use (&$expiredCount): void {
