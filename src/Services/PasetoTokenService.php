...
class PasetoTokenService implements TokenIssuer, TokenVerifier
{
    ...
    public function verifyOffline(string $token, string $publicKeyBundle): array
    {
        ...
        try {
            $publicKey = new AsymmetricPublicKey(base64_decode((string) $signingPublicKey), new Version4);
        } catch (Exception $e) {
            throw new RuntimeException('Invalid signing public key', $e->getCode(), $e);
        }
    ...
    protected function resolveClockSkew(?License $license): int
    {
        if ($license instanceof License) {
            return max(0, $license->getClockSkewSeconds());
        }
