...
## Entities
- `Simtabi\Laranail\Licence\Kit\Models\License` — issued license, polymorphic `licensable` morphTo.
- `Simtabi\Laranail\Licence\Kit\Models\LicenseUsage` — one consumed seat (device/VM/service/user/session).
- `Simtabi\Laranail\Licence\Kit\Models\LicenseRenewal` — extension records.
- `Simtabi\Laranail\Licence\Kit\Models\LicenseScope` — multi-product isolation.
- `Simtabi\Laranail\Licence\Kit\Models\LicenseTemplate` — prebuilt configuration linked to a scope.
- `Simtabi\Laranail\Licence\Kit\Models\LicenseTrial` — trial period tracking.
